...
router.post("/check", ensureAnyAuth, async (req, res) => {
    ...
    try {
        const geo_res = await geocoder.geocode( { zipcode: zip_code, country: "USA" });
        ...
        const match = neighborhood_centers.find( neighborhood_centers => {
            const distance = get_distance_in_miles(latitude, longitude, neighborhood_centers.lat, neighborhood_centers.lng);
            return distance <= 5;
        });
